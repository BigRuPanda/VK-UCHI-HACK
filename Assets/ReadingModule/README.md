...
| Браузер | Vosk STT | Примечание |
|---|---|---|
| Chrome 90+ | ✅ | Рекомендуется |
| Edge 90+ | ✅ | Рекомендуется |
| Safari 15+ | ⚠️ | Может требовать жест пользователя |
| Firefox | ⚠️ | Требует проверки WASM-поддержки |

---

## Устранение неполадок

**Слова накладываются друг на друга / слипаются в одну точку**  
→ Убедиться, что структура префаба двухслойная: корень `WordToken` + дочерний `WordVisual`.  
→ Проверить, что в компоненте **Word Token** поле **Visual Transform** указывает на `WordVisual`.  
→ Проверить, что на `SentenceContainer` есть **Horizontal Layout Group** с **Child Force Expand Width = Off**.

**Длинные слова наезжают на короткие**  
→ `Control Child Size Width = OFF` — установить **ON**.  
→ `Child Force Expand Width = ON` — установить **OFF**.

**Слова не видны / прозрачные**  
→ Убедиться, что поле **Label** в компоненте **Word Token** указывает на `TextMeshProUGUI` внутри `WordVisual`.

**Слова не выстраиваются в ряд, а складываются вертикально**  
→ Убедиться, что компонент называется **Horizontal** Layout Group.

**Полоска микрофона остаётся на 0**  
→ Проверить настройки ОС — нужный микрофон должен быть выбран и не заглушён.  
→ Открыть DevTools браузера и найти строки `[MicrophonePlugin]`.

**Панель разрешения появляется, но кнопка не реагирует**  
→ Страница должна раздаваться через **HTTPS** или `localhost`.

**`ReceivePermissionResult` / `ReceiveWord` никогда не вызываются**  
→ Убедиться, что GameObject называется **строго** `SpeechBridge`.

**Слова распознаются, но никогда не совпадают**  
→ Проверить `ReadingController.language` — должен совпадать с языком речи (`ru-RU`).  
→ Снизить `fuzzyMatchThreshold` до `0.60` для тестирования.

**Ошибки `DllImport` в редакторе**  
→ Нормально. `Microphone.cs` обёрнут в `#if UNITY_WEBGL && !UNITY_EDITOR`. Использовать клавиши M/Space/X.

**DoTween не найден**  
→ Импортировать с Asset Store. Убедиться, что пространство имён `DG.Tweening` разрешается.
