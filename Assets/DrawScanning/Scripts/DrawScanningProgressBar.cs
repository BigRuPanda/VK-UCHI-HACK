...
/// <summary>
/// DrawScanningProgressBar — UI helper for the DrawScanning module.
///
/// Attach to a GameObject that has a <see cref="Slider"/> and wire it to
/// <see cref="DrawScanningChecker"/> events via the Inspector.
///
...
/// </summary>
public class DrawScanningProgressBar : MonoBehaviour
{
    // ─────────────────────────────────────────────────────────────────────
    // Inspector fields
    // ─────────────────────────────────────────────────────────────────────
    ...
    [Header("References")]
    [Tooltip("The Slider whose value (0–1) represents the similarity score.")]
    ...
    [Tooltip("Optional: GameObject shown when the drawing is accepted (e.g. a star/checkmark).")]
    ...
    [Tooltip("Score (0-1) where the bar turns yellow. Since the score is normalized, 0.7 means 70% of the way to the accept threshold.")]
    [Range(0f, 1f)] public float midThreshold  = 0.70f;
    
    [Tooltip("Score (0-1) where the bar turns green. Since the score is normalized, 1.0 is the accept threshold.")]
    ...
    [Tooltip("Duration (seconds) for the acceptance flash animation.")]
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Public API — wire these to DrawScanningChecker events in Inspector
    // ─────────────────────────────────────────────────────────────────────

    /// <summary>
    /// Update the progress bar with the latest similarity score.
    /// Wire to <see cref="DrawScanningChecker.OnSimilarityUpdate"/>.
    /// </summary>
    ...
    /// <summary>
    /// Play the acceptance animation.
    /// Wire to <see cref="DrawScanningChecker.OnDrawingAccepted"/>.
    /// </summary>
    public void OnDrawingAccepted()
    {
        _accepted = true;
    ...
    private void ValidateReferences()
    {
        ...
        {
            Debug.LogWarning(
                "[DrawScanningProgressBar] progressSlider is not assigned and no Slider was found on this GameObject. " +
        ...
        {
            Debug.LogWarning(
                "[DrawScanningProgressBar] fillImage is not assigned. " +
